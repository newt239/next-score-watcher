...
describe("nomx形式のスコア計算", () => {
    ...
    const logs: LogDBProps[] = [
      {
        ...
        variant: "correct",
        system: 0,
        timestamp: cdate().format("YYYY-MM-DD HH:mm:ss"),
        available: 1,
      },
    ];
    const result = await nomx(mockGame, logs);
  ...
  it("勝ち抜けポイントに達すると勝利状態になる", async () => {
    const logs: LogDBProps[] = Array.from({ length: 7 }, (_, i) => ({
      ...
      system: 0,
      timestamp: cdate().add(i, "seconds").format("YYYY-MM-DD HH:mm:ss"),
      available: 1,
    }));
    const result = await nomx(mockGame, logs);
  ...
  it("正解と誤答が混在した場合に正しく計算される", async () => {
    const logs: LogDBProps[] = [
      ...
      },
      {
        ...
        variant: "wrong",
        system: 0,
        timestamp: cdate().add(1, "seconds").format("YYYY-MM-DD HH:mm:ss"),
        available: 1,
      },
      {
        id: "log3",
        ...
        variant: "correct",
        system: 0,
        timestamp: cdate().add(2, "seconds").format("YYYY-MM-DD HH:mm:ss"),
        available: 1,
      },
    ];
    const result = await nomx(mockGame, logs);
