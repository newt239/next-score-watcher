...
ユニットテストでは`getInitialPlayersStateForOnline`がz形式のプレイヤーを`stage: 1`、スコア0、正解数0、誤答数0で初期化し、リーチ状態と失格フラグをいずれも解除した状態で開始させることを確認する。`initialScore`に値を設定した場合でもステージとスコアが固定値で保たれること、多人数を初期化した際に並び順が全員横並びになることを検証し、ステージ値の違いが出現したログ適用後に正しい優先順位が得られる前提を担保する。
