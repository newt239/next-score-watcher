...
ユニットテストでは`getInitialPlayersStateForOnline`がsquarex形式で奇数軸・偶数軸のスコアを`initialScore`で初期化し、総合スコアをその二乗で設定することを確認する。`initialScore`が未設定の場合に各値が0で開始すること、初期値を与えた場合に`odd_score`と`even_score`が同じ値で始まり、`score`がその積に一致すること、さらに複数プレイヤーを用意して並び順判定がスコアと正解数を優先することを検証する。
