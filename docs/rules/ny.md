...
ユニットテストでは`getInitialPlayersStateForOnline`がny形式で常にスコア0を設定し、`initialScore`に応じて正解数と誤答数のみが初期化されることを確認する。正解数に初期値を持つプレイヤーと初期値なしのプレイヤーを混在させてもリーチ状態が`playing`で統一されること、また勝利・敗退状態が未確定の段階で`getSortedPlayerOrderListForOnline`が正解数優先で並び替えることを検証し、`generateScoreText`が通常時にスコア数値を返す挙動も合わせて担保する。
