...
ユニットテストでは`getInitialPlayersStateForOnline`がnormal形式のプレイヤーに対してスコア0、正解数と誤答数を`initialScore`で初期化し、状態・リーチ状態・解答権フラグをすべて`playing`または`false`で統一することを確認する。併せて、初期値を持つプレイヤーと未設定のプレイヤーを混在させたケースで、`getSortedPlayerOrderListForOnline`が勝ち抜け状態・ステージ・スコア・正解数・誤答数の優先順位どおりに並び替えること、さらに`generateScoreText`と`indicator`が勝者に順位テキストを、敗退者に`LOSE`を、通常状態にスコア数値を返すことを検証する。
