...
適切な粒度でコミットを行ってください。コミットメッセージは`prefix: message`の形式で1行以内に簡潔に記述してください。原則として括弧書きは使用しないでください。
...
- 自前の型ガード関数（`value is T`を返す関数）は使用しないでください。値のバリデーションや型の絞り込みが必要な場合は**Zod**のスキーマと`safeParse`を使用してください。
...
- 2回以下しか使われていない関数や、ほとんど他の関数を呼び出して終わっているだけの関数、`filter`や`map`をするだけの関数など、中身の薄い関数は関数化せず、直接呼び出し元で実装してください。
